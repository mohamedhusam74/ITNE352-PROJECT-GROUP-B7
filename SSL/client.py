...
def create_ssl_socket():
    ...
    context.load_verify_locations("server.crt")  # Load server's certificate
    ssl_client_socket = context.wrap_socket(client_socket, server_hostname="localhost")

    return ssl_client_socket

def main():
    """Establishes the SSL client-server connection and manages the main menu."""
    # Create an SSL socket
    client = create_ssl_socket()

    server_address = ("127.0.0.1", 7000)  # Match the server's address and port
    client.connect(server_address)  # Connect to the server securely
